...
float *last_xy = new float[2];
float *xRef = new float[41];
float *yRef = new float[41];
...
int main()
{
    ...
    for (int i = 0; i < 41; i++)
        {
	    yRef[i] = 0.25 * cos((float)0.025*PI*i*0.25) + 0.25;
            xRef[i] = 2 * sin((float)0.025*PI*i*0.25) + 0.2;
        }
	last_xy[0] = xRef[0];
	last_xy[1] = yRef[0];
	...
	while(outer_count < 41) {
		...
		act_coor = sv.GetCoordinates(last_xy[0], last_xy[1]);
		cout << "act x: " << act_coor[0] << "; ref x: " << xRef[outer_count] << endl << "act y: " << act_coor[1] << "; ref y: " << yRef[outer_count] << endl << endl;
		last_xy[0] = act_coor[0];
		last_xy[1] = act_coor[1];
		if (outer_count == 0) {act_coor[2] = 0;}
		...
		//Control		
		v = Kp_v * (double)sqrt(e_x * e_x + e_y * e_y) + Kd_v * 2 * (e_x * (double)(e_x - e_x_1) + e_y * (double)(e_y - e_y_1)) * I_ITV_OVER_TIME * 0.2 * signum(cos(e_phi));
		e_phi = atan(tan(e_phi));
		alpha = Kp_phi * e_phi + Kd_phi * (double)(e_phi - e_phi_1) * I_ITV_OVER_TIME * 0.2;
		if (abs(alpha) > 0.5026)
		{
		...
		outer_count++;	
		//sv.SendData(buf);
		//cout << "Left: " << w_left << "; Right: " << w_right << endl << endl;	
    }
    //sv.SendData(buffer);
    pwmWrite(IN1, 0);
    pwmWrite(IN3,0);
